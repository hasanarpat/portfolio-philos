...
export function Navigation() {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 z-30 transition-opacity duration-500 pointer-events-none ${
      ...
      <div className="max-w-7xl mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <Link
            href="/"
            className="font-mono text-sm uppercase tracking-widest text-primary hover:text-secondary transition-colors text-glow"
          >
            [System.Init]
          </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`relative font-mono text-xs uppercase tracking-wider transition-all duration-300 group ${
                    isActive ? "text-primary text-glow" : "text-foreground/60 hover:text-primary"
                  }`}
                >
          ...
          >
            <span
              className={`absolute left-2 right-2 top-3 h-[2px] bg-primary transition-all duration-300 ${
                menuOpen ? "translate-y-[5px] rotate-45" : ""
              }`}
            />
            <span
              className={`absolute left-2 right-2 top-5 h-[2px] bg-primary transition-all duration-300 ${
                menuOpen ? "opacity-0" : "opacity-100"
              }`}
            />
            <span
              className={`absolute left-2 right-2 top-7 h-[2px] bg-primary transition-all duration-300 ${
                menuOpen ? "-translate-y-[5px] -rotate-45" : ""
              }`}
            />
          </button>
      ...
      >
        ...
        />
        <div
          className={`absolute left-4 right-4 top-20 border border-primary/30 bg-background/90 text-primary transition-all duration-500 ${
              ...
              {navItems.map((item, index) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setMenuOpen(false)}
                    className={`group flex items-center justify-between border border-primary/20 px-4 py-3 font-mono text-xs uppercase tracking-widest transition-all duration-300 ${
                      isActive
            ...
            <div className="relative border-t border-primary/20 px-4 py-3 text-[10px] font-mono text-muted-foreground">
              ...
              <div className="mt-2 flex items-center gap-2">
                {Array.from({ length: 14 }).map((_, i) => (
                  <span
                    key={`pulse-${i}`}
                    className={`block h-1 w-2 rounded-sm border border-primary/20 ${
